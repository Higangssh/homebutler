...
- adding a second notification channel rewrote the section header (#154). The config writer wrote the whole path of a key it could not find, so writing `notify.gotify.url` into a file that already had `notify:` produced a second `notify:` below the first. The result did not parse, the save was refused, and nothing could be added to a section that already existed — which is every channel after the first. It now writes only the part of the path that is missing, under the deepest key that is already there, including a section left empty by removing its last channel
